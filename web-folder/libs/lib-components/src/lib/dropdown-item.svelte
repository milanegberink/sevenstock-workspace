...
{#each items as item, i (item)}
	{#if item.items}
		...
		<div
			...
			class="bottom-anchor-bottom left-anchor-right border-primary bg-primary hidden -translate-x-10 flex-col gap-1 rounded-2xl border p-1 opacity-0 transition-all transition-discrete duration-100 open:flex open:-translate-x-0 open:opacity-100 starting:open:-translate-x-10 starting:open:opacity-0"
		>
